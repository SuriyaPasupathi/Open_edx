...
def change_enrollment(request, check_access=True):
    ...
    if action == "enroll":
        ...
        # Always redirect to courseware after successful enrollment
        # This ensures users are taken to the course view section after enrolling
        try:
            courseware_url = make_learning_mfe_courseware_url(course_id)
        except Exception:
            # Fallback to traditional courseware URL if MFE URL generation fails
            courseware_url = reverse('courseware', args=[str(course_id)])
        return HttpResponse(courseware_url)
