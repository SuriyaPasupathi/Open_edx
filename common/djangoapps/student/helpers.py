...
def get_next_url_for_login_page(request, include_host=False):
    ...
    log.info(f"REDIRECT DEBUG - initial redirect_to: {redirect_to}")
    log.info(f"REDIRECT DEBUG - ROOT_URLCONF: {settings.ROOT_URLCONF}")
    
    if not redirect_to:
        if settings.ROOT_URLCONF == 'lms.urls':
            login_redirect_url = configuration_helpers.get_value('DEFAULT_REDIRECT_AFTER_LOGIN')
            log.info(f"REDIRECT DEBUG - DEFAULT_REDIRECT_AFTER_LOGIN: {login_redirect_url}")

            if login_redirect_url:
                try:
                    redirect_to = reverse(login_redirect_url)
                    log.info(f"REDIRECT DEBUG - reversed login_redirect_url: {redirect_to}")
            ...
            if not redirect_to:
                ...
                log.info(f"REDIRECT DEBUG - default dashboard redirect: {redirect_to}")
            
            # Force redirect to dashboard if still not set
            if not redirect_to:
                redirect_to = '/dashboard'
                log.info(f"REDIRECT DEBUG - forced dashboard redirect: {redirect_to}")
