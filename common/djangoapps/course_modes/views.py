...
class ChooseModeView(View):
    ...
    def post(self, request, course_id):
        ...
        if requested_mode == 'verified':
            ...
            # Skip payment requirement - allow free verified enrollment
            # Set amount to 0 to bypass payment
            amount_value = decimal.Decimal(0)
            
            donation_for_course = request.session.get("donation_for_course", {})
            LOG.info(
                '[Track Selection Check] Donation for course [%s]: [%s], amount value: [%s] (payment bypassed)',
                course_id, donation_for_course, amount_value
            )
            donation_for_course[str(course_key)] = amount_value
            request.session["donation_for_course"] = donation_for_course

            # Directly enroll in verified mode without payment
            from common.djangoapps.student.models import CourseEnrollment
            from django.urls import reverse
            from django.shortcuts import redirect
            ...
            if enrollment:
                ...
                CourseEnrollment.enroll(request.user, course_key, mode='verified')
            
            # Skip verification URL redirect and go directly to course
            return redirect(reverse('courseware', args=[str(course_key)]))
