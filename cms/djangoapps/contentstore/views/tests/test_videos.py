...
class VideoUploadPostTestsMixin:
    ...
    @patch('cms.djangoapps.contentstore.video_storage_handlers.boto3.client')
    def test_post_success(self, mock_boto3_client):
        files = [
            {
                'file_name': 'first.mp4',
        ...
        # Verify boto3.client was called with correct credentials
        mock_boto3_client.assert_called()
        call_kwargs = mock_boto3_client.call_args[1]
        self.assertEqual(call_kwargs['aws_access_key_id'], settings.AWS_ACCESS_KEY_ID)
        self.assertEqual(call_kwargs['aws_secret_access_key'], settings.AWS_SECRET_ACCESS_KEY)
        ...
        for i, file_info in enumerate(files):
            ...
            call_args, call_kwargs = mock_s3_client.generate_presigned_url.call_args_list[i]
            self.assertEqual(call_args[0], 'put_object')
            ...
            # Verify bucket
            self.assertEqual(params['Bucket'], settings.VIDEO_UPLOAD_PIPELINE['VEM_S3_BUCKET'])
            
            # Verify content type
            self.assertEqual(params['ContentType'], file_info['content_type'])
...
class VideosHandlerTestCase(
    ...
    @ddt.unpack
    def test_video_supported_file_formats(self, files, expected_status, mock_boto3_client):
        """
    ...
    @override_settings(AWS_ACCESS_KEY_ID='test_key_id', AWS_SECRET_ACCESS_KEY='test_secret')
    @patch('cms.djangoapps.contentstore.video_storage_handlers.boto3.client')
    def test_upload_with_non_ascii_charaters(self, mock_boto3_client):
        """
        Test that video uploads throws error message when file name contains special characters.
        """
        # This test doesn't actually need a mock since it fails before S3 is called
        mock_s3_client = Mock()
        mock_boto3_client.return_value = mock_s3_client
    ...
    def test_devstack_upload_connection(self, mock_boto3_client):
        files = [{'file_name': 'first.mp4', 'content_type': 'video/mp4'}]
        mock_s3_client = Mock()
        mock_s3_client.generate_presigned_url.return_value = 'http://example.com/url_first.mp4'
        mock_boto3_client.return_value = mock_s3_client
        ...
        self.assertEqual(response.status_code, 200)
        # Verify boto3.client was called with correct credentials including session token
        mock_boto3_client.assert_called()
        call_kwargs = mock_boto3_client.call_args[1]
        self.assertEqual(call_kwargs['aws_access_key_id'], settings.AWS_ACCESS_KEY_ID)
        self.assertEqual(call_kwargs['aws_secret_access_key'], settings.AWS_SECRET_ACCESS_KEY)
        self.assertEqual(call_kwargs['aws_session_token'], settings.AWS_SECURITY_TOKEN)

    @patch('cms.djangoapps.contentstore.video_storage_handlers.boto3.client')
    def test_send_course_to_vem_pipeline(self, mock_boto3_client):
        """
        ...
        self.assertEqual(response.status_code, 200)
        # Verify that generate_presigned_url was called with VEM_S3_BUCKET
        mock_s3_client.generate_presigned_url.assert_called()
        call_kwargs = mock_s3_client.generate_presigned_url.call_args[1]
        self.assertEqual(call_kwargs['Params']['Bucket'], settings.VIDEO_UPLOAD_PIPELINE['VEM_S3_BUCKET'])
    ...
    def test_video_upload_token_in_meta(self, data, mock_boto3_client):
        """
        ...
        mock_s3_client = Mock()
        mock_s3_client.generate_presigned_url.return_value = 'http://example.com/url_{}'.format(file_data['file_name'])
        mock_boto3_client.return_value = mock_s3_client
        ...
        with patch.object(WaffleFlagCourseOverrideModel, 'override_value', return_value=data['course_override']):
            with override_waffle_flag(DEPRECATE_YOUTUBE, active=data['global_waffle']):
                ...
                with proxy_manager(self.assertRaises(AssertionError), data['expect_token']):
                    ...
                    # if we're expecting token then we will be able to find the token in s3 metadata.
                    # With boto3, metadata is passed to generate_presigned_url in Params['Metadata']
                    call_kwargs = mock_s3_client.generate_presigned_url.call_args[1]
                    if data['expect_token']:
                        self.assertIn('course_video_upload_token', call_kwargs['Params']['Metadata'])
                        self.assertEqual(call_kwargs['Params']['Metadata']['course_video_upload_token'], self.test_token)
                    else:
                        self.assertNotIn('course_video_upload_token', call_kwargs['Params']['Metadata'])
...
class TranscriptPreferencesTestCase(VideoUploadTestBase, CourseTestCase):
    ...
    def test_transcript_preferences_metadata(self, transcript_preferences, is_video_transcript_enabled,
                                             mock_transcript_preferences, mock_boto3_client):
        """
        ...
        mock_s3_client = Mock()
        mock_s3_client.generate_presigned_url.return_value = f'http://example.com/url_{file_name}'
        mock_boto3_client.return_value = mock_s3_client
        ...
        # With boto3, metadata is passed to generate_presigned_url in Params['Metadata']
        call_kwargs = mock_s3_client.generate_presigned_url.call_args[1]
        metadata = call_kwargs['Params']['Metadata']
...
class GetStorageBucketTestCase(TestCase):
    ...
    @patch('cms.djangoapps.contentstore.video_storage_handlers.boto3.client')
    def test_storage_bucket(self, mock_boto3_client):
        ...
        mock_s3_client.generate_presigned_url.return_value = (
            "https://vem_test_bucket.s3.amazonaws.com/test_root/dummy_video"
        )
        mock_boto3_client.return_value = mock_s3_client

        from cms.djangoapps.contentstore.video_storage_handlers import _get_s3_client

        bucket = storage_service_bucket()
        edx_video_id = 'dummy_video'
        key_info = storage_service_key(bucket, file_name=edx_video_id)
        ...
        self.assertIn("test_root", upload_url)
