...
def storage_service_bucket():
    """
    Returns an S3 bucket name for video upload.
    Note: With boto3, we return the bucket name instead of a bucket object.
    """
...
def _get_s3_client():
    """
    Returns a boto3 S3 client configured with the appropriate credentials.
    """
    if ENABLE_DEVSTACK_VIDEO_UPLOADS.is_enabled():
        client_kwargs = {
            'aws_access_key_id': settings.AWS_ACCESS_KEY_ID,
            'aws_secret_access_key': settings.AWS_SECRET_ACCESS_KEY,
            'aws_session_token': settings.AWS_SECURITY_TOKEN
        }
    else:
        client_kwargs = {
            'aws_access_key_id': settings.AWS_ACCESS_KEY_ID,
    ...
    return boto3.client('s3', config=Config(signature_version='s3v4'), **client_kwargs)


def storage_service_key(bucket, file_name):
    """
    Returns an S3 key name for the given file in the given bucket.
    With boto3, we return a dict with bucket and key_name instead of a Key object.
    """
    key_name = "{}/{}".format(
