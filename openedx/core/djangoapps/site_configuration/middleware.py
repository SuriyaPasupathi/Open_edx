...
class SessionCookieDomainOverrideMiddleware(MiddlewareMixin):
    ...
    def process_response(self, __, response):
        ...
        # CRITICAL FIX: Always wrap set_cookie to fix SameSite issues for browser compatibility
        def _set_cookie_wrapper_always(key, value='', max_age=None, expires=None, path='/', domain=None, secure=None,
                                httponly=False, samesite=None):
            """
            Wrapper function for set_cookie() which fixes SameSite=None issues
            """
            # Fix SameSite=None for session cookies (browsers reject this over HTTP)
            ...
            # Call the original set_cookie method
            return response.set_cookie_wrapped_func(key, value, **kwargs)
        
        # Always apply the SameSite fix
        if not hasattr(response, 'set_cookie_wrapped_func'):
            response.set_cookie_wrapped_func = response.set_cookie
            response.set_cookie = _set_cookie_wrapper_always
        ...
        if session_cookie_domain:
            def _set_cookie_wrapper(key, value='', max_age=None, expires=None, path='/', domain=None, secure=None,
                ...
                # CRITICAL FIX: Force SameSite=Lax for ALL session cookies for browser compatibility
                # Modern browsers reject SameSite=None over HTTP
                if key == configuration_helpers.get_value('SESSION_COOKIE_NAME', settings.SESSION_COOKIE_NAME):
                    if samesite == 'None':
                        samesite = 'Lax'
