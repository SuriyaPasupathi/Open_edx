...
def _get_user_by_email_or_username(request, api_version):
    ...
    # Handle both request.POST (form data) and request.data (JSON data from API)
    data_source = request.POST
    if hasattr(request, 'data') and request.data:
        data_source = request.data

    if any(f not in data_source.keys() for f in login_fields):
        raise AuthFailedError(_("There was an error receiving your login information. Please email us."))

    email_or_username = data_source.get("email", None) or data_source.get("email_or_username", None)
...
def _enforce_password_policy_compliance(request, user):  # lint-amnesty, pylint: disable=missing-function-docstring
    try:
        ...
        if hasattr(request, 'data') and request.data:
            data_source = request.data
        password_policy_compliance.enforce_compliance_on_login(user, data_source.get("password"))
...
def _authenticate_first_party(request, unauthenticated_user, third_party_auth_requested):
    ...
    password = normalize_password(data_source["password"])
...
def _handle_successful_authentication_and_login(user, request):
    ...
    try:
        ...
        log.debug(f"Session saved. Session key: {request.session.session_key}")
        log.debug(f"Session data: {dict(request.session)}")
...
def _track_user_login(user, request):
    ...
    segment.track(
        ...
        {
            "category": "conversion",
            "label": data_source.get("course_id"),
            "provider": None,
...
def login_user(request, api_version="v1"):  # pylint: disable=too-many-statements
    ...
    first_party_auth_requested = any(bool(data_source.get(p)) for p in ["email", "email_or_username", "password"])
    is_user_third_party_authenticated = False

    set_custom_attribute("login_user_course_id", data_source.get("course_id"))
    ...
    try:
        ...
        if is_user_third_party_authenticated:
            ...
            # Default redirect for regular API login - create full URL
            next_url, root_url = get_next_url_for_login_page(request, include_host=True)
            redirect_url = get_redirect_url_with_host(root_url, "/dashboard")
            log.debug(f"Setting default redirect_url to: {redirect_url}")
        ...
        log.debug(f"Final redirect_url being returned: {redirect_url}")
        ...
        # Debug: Check if session cookie will be set
        log.debug(f"Session key before response: {request.session.session_key}")
        log.debug(f"Session modified: {request.session.modified}")
        log.debug(f"Session accessed: {request.session.accessed}")
        
        # Force session to be considered modified so cookie gets set
        if request.session.session_key:
            request.session.modified = True
            log.debug("Forced session.modified = True")

        # Ensure that the external marketing site can
        # detect that the user is logged in.
        response = set_logged_in_cookies(request, response, possibly_authenticated_user)
        
        # Note: Custom response wrapper approach failed due to Django REST Framework requirements
        # Will handle SameSite issue via middleware or settings instead
        ...
        email_or_username = data_source.get(email_or_username_key, None)
...
def _parse_analytics_param_for_course_id(request):
    ...
    modified_request = data_source.copy()
