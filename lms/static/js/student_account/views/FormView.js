        ...
        return Backbone.View.extend({
            ...
            */
            ...
            initialize: function(data) {
                console.log('[FormView] FormView initializing with data:', data);
                ...
                console.log('[FormView] FormView initialization complete');
                
                // Debug: Check if events are bound
                console.log('[FormView] Events object:', this.events);
                console.log('[FormView] Form element:', this.$form);
                console.log('[FormView] Submit button selector:', this.submitButton);
                
                // Debug: Check if submit button exists
                var $submitButton = this.$form.find(this.submitButton);
                console.log('[FormView] Submit button found:', $submitButton.length > 0);
                console.log('[FormView] Submit button element:', $submitButton);
            },
            ...
            getFormData: function() {
                console.log('[FormView.getFormData] ===== GET FORM DATA START =====');
                var obj = {},
                    $form = this.$form,
                ...
                console.log('[FormView.getFormData] Form elements count:', len);
                ...
                for (i = 0; i < len; i++) {
                    ...
                    if (key) {
                        console.log('[FormView.getFormData] Processing field:', key, 'Value:', $el.val());
                        if (this.interesting_fields($el)) {
                            this.remove_validation_error($el, $form);
                        }
                        validation = this.validate(elements[i]);
                        console.log('[FormView.getFormData] Validation result for', key, ':', validation);
                        if (validation.isValid) {
                            obj[key] = $el.attr('type') === 'checkbox' ? $el.is(':checked') : $el.val();
                            $el.removeClass('error');
                            $label.removeClass('error');
                            console.log('[FormView.getFormData] Field', key, 'is valid, added to obj');
                        } else {
                            console.log('[FormView.getFormData] Field', key, 'is INVALID:', validation.message);
                            if (this.interesting_fields($el)) {
                ...
                console.log('[FormView.getFormData] Final errors:', this.errors);
                console.log('[FormView.getFormData] Final obj:', obj);
                console.log('[FormView.getFormData] ===== GET FORM DATA END =====');
            ...
            submitForm: function(event) {
                console.log('[FormView.submitForm] ===== FORM SUBMISSION START =====');
                var data = this.getFormData();
                console.log('[FormView.submitForm] Form data:', data);

                if (!_.isUndefined(event)) {
                    event.preventDefault();
                    console.log('[FormView.submitForm] Prevented default form submission');
                }

                this.toggleDisableButton(true);
                console.log('[FormView.submitForm] Disabled submit button');

                if (!_.compact(this.errors).length) {
                    console.log('[FormView.submitForm] No validation errors, proceeding with submission');
                    data = this.setExtraData(data);
                    console.log('[FormView.submitForm] Final data to submit:', data);
                    this.model.set(data);
                    console.log('[FormView.submitForm] Set data on model, calling model.save()');
                    this.model.save();
                    this.clearFormErrors();
                } else {
                    console.error('[FormView.submitForm] Validation errors found:', this.errors);
                    this.renderErrors(this.defaultFormErrorsTitle, this.errors);
                    this.scrollToFormFeedback();
                    this.toggleDisableButton(false);
                }
                console.log('[FormView.submitForm] ===== FORM SUBMISSION END =====');
