    ...
    function($, utility, _, _s, Backbone, LoginModel, PasswordResetModel, RegisterModel, AccountRecoveryModel,
        ...
        return Backbone.View.extend({
            ...
            initialize: function(options) {
                console.log('[AccessView] AccessView initializing with options:', options);
            ...
            load: {
                login: function(data) {
                    ...
                    console.log('[AccessView.load.login] Setting up auth-complete event listener');
                    console.log('[AccessView.load.login] isEnterpriseEnable:', this.isEnterpriseEnable);
                    console.log('[AccessView.load.login] isTpaSaml:', isTpaSaml);
                    
                    if (this.isEnterpriseEnable == true && !isTpaSaml) {
                        console.log('[AccessView.load.login] Using loginComplete handler');
                        this.listenTo(this.subview.login, 'auth-complete', this.loginComplete);
                    } else {
                        console.log('[AccessView.load.login] Using authComplete handler');
                        this.listenTo(this.subview.login, 'auth-complete', this.authComplete);
                    }
                    console.log('[AccessView.load.login] auth-complete event listener set up');
                },
            ...
            authComplete: function() {
                console.log('[AccessView.authComplete] ===== AUTH COMPLETE CALLBACK START =====');
                console.log('[AccessView.authComplete] this.thirdPartyAuth:', this.thirdPartyAuth);
                console.log('[AccessView.authComplete] this.nextUrl:', this.nextUrl);
                
                if (this.thirdPartyAuth && this.thirdPartyAuth.finishAuthUrl) {
                    console.log('[AccessView.authComplete] Using third party auth finish URL:', this.thirdPartyAuth.finishAuthUrl);
                    this.redirect(this.thirdPartyAuth.finishAuthUrl);
                    // Note: the third party auth URL likely contains another redirect URL embedded inside
                } else {
                    console.log('[AccessView.authComplete] No third party auth, using nextUrl');
                    // Fallback: if nextUrl is root or empty, send to dashboard
                    if (!this.nextUrl || this.nextUrl === '/' || this.nextUrl === '') {
                        console.log('[AccessView.authComplete] nextUrl is empty or root, setting to /dashboard');
                        this.nextUrl = '/dashboard';
                    }
                    console.log('[AccessView.authComplete] Final nextUrl for redirect:', this.nextUrl);
                    this.redirect(this.nextUrl);
                }
                console.log('[AccessView.authComplete] ===== AUTH COMPLETE CALLBACK END =====');
            },
            ...
            loginComplete: function() {
                console.log('[AccessView.loginComplete] ===== LOGIN COMPLETE CALLBACK START =====');
                console.log('[AccessView.loginComplete] this.thirdPartyAuth:', this.thirdPartyAuth);
                console.log('[AccessView.loginComplete] this.nextUrl:', this.nextUrl);
                console.log('[AccessView.loginComplete] this.edxUserInfoCookieName:', this.edxUserInfoCookieName);
                
                if (this.thirdPartyAuth && this.thirdPartyAuth.finishAuthUrl) {
                    console.log('[AccessView.loginComplete] Using third party auth finish URL:', this.thirdPartyAuth.finishAuthUrl);
                    console.log('[AccessView.loginComplete] Calling multipleEnterpriseInterface.check with finishAuthUrl');
                    ...
                    console.log('[AccessView.loginComplete] No third party auth, using nextUrl:', this.nextUrl);
                    console.log('[AccessView.loginComplete] Calling multipleEnterpriseInterface.check with nextUrl');
                    multipleEnterpriseInterface.check(this.nextUrl, this.edxUserInfoCookieName);
                }
                console.log('[AccessView.loginComplete] ===== LOGIN COMPLETE CALLBACK END =====');
            },
            ...
            redirect: function(url) {
                console.log('[AccessView.redirect] ===== REDIRECT FUNCTION CALLED =====');
                console.log('[AccessView.redirect] URL to redirect to:', url);
                console.log('[AccessView.redirect] Current window.location:', window.location.href);
                console.log('[AccessView.redirect] Calling window.location.replace...');
                window.location.replace(url);
                console.log('[AccessView.redirect] window.location.replace called');
                console.log('[AccessView.redirect] ===== REDIRECT FUNCTION END =====');
            },
