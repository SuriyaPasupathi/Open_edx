...
AUDIT_PASSING_CERT_DATA = CertData(
    ...
    _('You are enrolled in the audit track for this course. You will receive a certificate upon successful completion.'),
    download_url=None,
...
class EnrollStaffView(View):
    ...
    def post(self, request, course_id):
        ...
        if enroll:
            ...
            # Redirect to Learning MFE if enabled
            if settings.FEATURES.get('ENABLE_LEARNING_MICROFRONTEND'):
                from openedx.features.course_experience.url_helpers import make_learning_mfe_courseware_url
                return redirect(make_learning_mfe_courseware_url(course_key))
            else:
                _next = urllib.parse.quote_plus(request.GET.get('next', reverse('courseware', args=[str(course_key)])), safe='/:?=')
...
def course_about(request, course_id):  # pylint: disable=too-many-statements
    ...
    with modulestore().bulk_operations(course_key):
        ...
        # Allow authenticated users to enroll if enrollment is open (no approval required)
        # This enables enrollment for all registered users without admin/CMS approval
        can_enroll = bool(
            request.user.has_perm(ENROLL_IN_COURSE, course) or
            (request.user.is_authenticated and course.is_enrollment_open())
        )
...
def _progress(request, course_key, student_id):
    ...
    # Get completion summary for progress tracking for all users
...
def get_cert_data(student, course, enrollment_mode, course_grade=None):
    ...
    # Check if certificates are viewable for the course
