...
def _can_enroll_courselike(user, courselike):
    ...
    # Allow authenticated users to enroll if enrollment is open, regardless of invitation_only status
    # This enables enrollment for all registered users without admin/CMS approval
    if user is not None and user.is_authenticated and course_enrollment_open:
        debug("Allow: authenticated user in enrollment period (approval not required)")
        return ACCESS_GRANTED

    # Access denied when the course requires an invitation (only for anonymous users)
