    ...
    def save_history_entry(student_module, history_model_cls, request_cache_key):
        ...
        if student_module.module_type in history_model_cls.HISTORY_SAVING_TYPES:
            ...
            try:
                ...
                # Gracefully handle database errors (e.g., table doesn't exist, connection issues)
                # Log the error but don't fail the request
                from django.db.utils import ProgrammingError, OperationalError
                if isinstance(exc, (ProgrammingError, OperationalError)):
                    log.warning(
                        "Failed to save history entry for StudentModule(%s) - history table may not exist or be configured: %s",
                        student_module.id,
                        str(exc)
                    )
                else:
